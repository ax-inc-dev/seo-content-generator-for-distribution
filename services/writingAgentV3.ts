...
emphasis_rules:
  ...
  heading_restriction: |
    【重要】見出しタグ内での<b>使用禁止
    - <h2>〜</h2>タグの中では<b>タグを使用しない
    - <h3>〜</h3>タグの中では<b>タグを使用しない
    - 見出しタグ以降の本文（<p>タグ内など）では<b>タグの使用を推奨
    
    例：
    ❌ 悪い例: <h2>AIで<b>業務効率化</b>を実現</h2>
    ✅ 良い例: <h2>AIで業務効率化を実現</h2>
    ✅ 良い例: <p>AIは<b>業務効率化</b>に大きく貢献します。</p>
...
instruction: "接続詞を使って文章を自然につなぐ。ラベル付けではなく文脈で論理を示す"
  accessibility: "画像内テキストの要点は本文でも説明"
  deliverable: "本文中に『図のキャプション案』『alt案』をセットで明示（実画像は出力しない）"
