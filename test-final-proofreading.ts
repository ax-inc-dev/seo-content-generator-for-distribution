// 最終校閲エージェント本番テスト（TypeScript版）
import { FinalProofreadingAgent } from './services/finalProofreadingAgent';
import type { FinalProofreadRequest } from './services/finalProofreadingAgent';
...
console.log('🔑 APIキー検出:', process.env.OPENAI_API_KEY?.substring(0, 20) + '...\n');
...
async function testFinalProofreading() {
  try {
    ...
    // 3. 完全な校閲実行（オプション）
    console.log('\n\n===== 3. 完全校閲テスト（簡易版） =====');
    const proofreadRequest: FinalProofreadRequest = {
      article: testArticle,
      keyword: 'GPT-5 AI',
    ...
    if (error.status === 400) {
      ...
      console.log('  詳細:', error.response?.data || error);
    }
